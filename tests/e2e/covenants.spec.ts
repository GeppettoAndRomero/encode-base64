...
test.describe('covenants', () => {
  ...
  test('links to related tools and its own engineering-notes article (#177)', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      expect(href, `related card ${i} is not a self-link`).not.toContain('/encode-base64/');
    ...
    if (await blogLink.count()) {
      await expect(blogLink).toHaveAttribute('href', /^https:\/\/runlocally\.app\/blog\/encode-base64\/$/);
    }
  });
