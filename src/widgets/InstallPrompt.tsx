...
export function InstallPrompt({ locale = 'en' }: InstallPromptProps) {
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      // 保存された表示モードを確認
      const savedMode = localStorage.getItem('encode-base64-install-prompt-mode') as DisplayMode | null;
      const dismissedTime = localStorage.getItem('encode-base64-install-prompt-dismissed');

      // 1週間経過していたらリセット
      if (dismissedTime) {
        const oneWeek = 7 * 24 * 60 * 60 * 1000;
        if (Date.now() - parseInt(dismissedTime, 10) >= oneWeek) {
          localStorage.removeItem('encode-base64-install-prompt-mode');
          localStorage.removeItem('encode-base64-install-prompt-dismissed');
  ...
  const handleInstall = async () => {
    ...
    setDisplayMode('hidden');
    localStorage.setItem('encode-base64-install-prompt-mode', 'hidden');
  };

  const handleDismissBanner = () => {
    // バナーを閉じてフッターモードに移行
    setDisplayMode('footer');
    localStorage.setItem('encode-base64-install-prompt-mode', 'footer');
  };
  ...
  const handleDismissFooter = () => {
    ...
    setDisplayMode('hidden');
    localStorage.setItem('encode-base64-install-prompt-mode', 'hidden');
    localStorage.setItem('encode-base64-install-prompt-dismissed', Date.now().toString());
  };
